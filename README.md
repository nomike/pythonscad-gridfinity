...
These previews are generated by the deterministic
`scripts/render_gallery_*.py` models using PythonSCAD.
...
| --- | --- | --- |
| ![Weighted 2×2 baseplate](docs/gallery/baseplate.png) | ![2×1 bin with compartments](docs/gallery/bin.png) | ![2×1 vase bin](docs/gallery/vase.png) |

## Dependencies

- **Runtime:** [PythonSCAD](https://pythonscad.org/) **1.1.0 or newer**
  (`pythonscad>=1.1.0` on PyPI)
- **Python:** 3.10+
...
When opening examples from a clone without installing, add `src/` to the path
(see `examples/basic_baseplate.py`).

### Use it as a project dependency

Pin the release in `requirements.txt`:

```text
pythonscad-gridfinity==0.1.0
```
...
If your script lives inside the repository (e.g. in `examples/`), use
...
See [`spec.py`](src/gridfinity/spec.py) for the full list.
...
[VERSIONING.md](VERSIONING.md) for releases, and
[PUBLISHING.md](PUBLISHING.md) for GitHub and PyPI trusted publishing.
